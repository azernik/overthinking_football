...
give.n <- function(x){
  return(c(y = median(x)*2, label = length(unique(x$Player)))) 
  # experiment with the multiplier to find the perfect position
}

 ggplot(all_combined_Team_Pos_min_10_top_20, aes(x = reorder(Team_space_Pos, xPts, FUN = median ), y = xPts, fill = Team.x)) + geom_boxplot()  + theme_minimal() + theme(axis.text.x = element_text(angle = 90)) + ggtitle("xFpts for the top 20 Team-Positions") + xlab("Team-Position") + ylab("xFpts") + colScale + fillScale +  stat_summary(fun.data = give.n, geom = "text", fun.y = median)
